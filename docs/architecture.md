...
- The read-only **`DocumentRepository`** boundary exposes metadata-only library listings and `LibrarySearchResult` values separately from an explicit full `ReaderDocument` load. `LocalDocumentRepository` currently adapts the existing persistence implementations; storage migration or a second database is not implied by this protocol.
- `DocumentPersisting.storageSizeInBytes()` reports the local store footprint when the adapter can measure it. Settings presents this as library storage; imported source files are never counted or deleted by library actions.
