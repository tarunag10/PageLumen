...
- [ ] Render only scanned/no-embedded-text pages, downsample to a documented OCR target DPI, and release each bitmap immediately after recognition. The PDF OCR path now targets roughly 144 DPI with a 50M-pixel cap and releases each page immediately; standalone image target-DPI normalization remains open.
