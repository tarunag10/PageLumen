...
- [ ] Add XCUITest coverage for import, review queue, export confirmation, settings, dark/light appearance, and denied permission recovery. The deterministic `-ui-testing-fixture` flow now loads the bounded demo and covers Review Queue/Continue plus every export format, export option, and preview without invoking a file panel; a `-ui-testing-settings` seam now covers privacy, searchable-copy, appearance, contrast, and retention controls; dedicated `-ui-testing-appearance-system`, `-ui-testing-appearance-light`, and `-ui-testing-appearance-dark` launches assert all three appearance choices without mutating persisted settings; `-ui-testing-import` and `-ui-testing-import-denied` now cover bounded import and explicit retry state transitions without invoking `NSOpenPanel` or macOS permissions. Real picker/permission, export confirmation/file writing, import/export execution, and participant execution remain open.
