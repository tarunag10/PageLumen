...
- [ ] Implement Finder Quick Action / Share support for selected PDFs and images. The app now declares PDF/image `CFBundleDocumentTypes`, filters unsupported Finder-open URLs through `PageLumenSystemWorkflowContract`, and bridges accepted files into the normal security-scoped import path; Finder extension/share-sheet packaging and participant validation remain open.
...
- [ ] Support a deliberate “summarise selection,” “explain table,” “describe figure,” “study notes,” and “compare passages” set before adding an open-ended chat interface. Selection-aware summarisation is now implemented; the remaining task modes are open.
