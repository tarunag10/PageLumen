...
- [ ] Implement Finder Quick Action / Share support for selected PDFs and images. The app now declares PDF/image `CFBundleDocumentTypes`, filters unsupported Finder-open URLs through `PageLumenSystemWorkflowContract`, and includes a bounded `PageLumenShareExtension` target that forwards accepted URLs through the app's normal open-file path; Finder Quick Action packaging and Finder/share/revoked-permission participant validation remain open.
- [ ] Add Quick Look thumbnails/previews only after verifying sandbox and performance behaviour. `PageLumenQuickLookExtension` now provides a bounded first-page/image thumbnail target with a 512-pixel cap; Quick Look preview, sandbox/performance validation, and participant testing remain open.
