...
- [ ] Add document comparison for source vs OCR and user-selected document revisions; never use AI comparison without provenance/citation output. `DocumentComparison` provides deterministic stable-block diffs, Review exposes a non-AI Compare Edits popover for retained original OCR, and `DocumentStore.comparisonChanges(comparedToRevision:)` now provides a non-mutating undo-revision baseline; visible revision browsing remains open.
