...
- [ ] Add XCUITest coverage for import, review queue, export confirmation, settings, dark/light appearance, and denied permission recovery. The deterministic `-ui-testing-fixture` flow now loads the bounded demo and covers Review Queue/Continue plus export controls; a `-ui-testing-settings` seam now covers privacy, searchable-copy, appearance, contrast, and retention controls; `-ui-testing-import` and `-ui-testing-import-denied` now cover bounded import and explicit retry state transitions without invoking `NSOpenPanel` or macOS permissions. Real picker/permission, import/export, and participant execution remain open.
