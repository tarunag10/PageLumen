...
- [ ] Preserve the fast embedded-text path but augment it with PDF semantics: page labels, links, bookmarks, metadata, annotations, form fields, and text positions when available. PDF document metadata, human-facing page labels, nested bookmarks, and link annotations (bounds, labels, URLs/internal targets) are now retained through the model and legacy Codable decoding; non-link annotations, form fields, and text positions remain open.
