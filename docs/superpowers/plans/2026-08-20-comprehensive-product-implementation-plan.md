...
- [x] If SwiftData cannot initialize, use `FilePersisting` in a safe app-support subdirectory and retain the active in-memory document while surfacing degraded status. Read-only and no-space behavior is covered by injected-writer tests; physical release-build fallback and migration runs remain open.
...
- [x] Add JSON-store corruption recovery: preserve the damaged file under a
  ...
  in-memory document. Read-only/no-space behavior is covered by deterministic
  tests; migration and physical release fallback remain separate gates.
...
- [x] Add tests for error mapping, no selected window, cancellation, temp cleanup, and command argument construction. `ScreenshotCaptureServiceTests` covers injected permission/picker cancellation and no-shareable-content paths, command failure, cleanup, and exact legacy arguments without invoking TCC; real picker UI and macOS 14/current-release participant validation remain separate manual gates.
...
- [ ] Verify system Increase Contrast, Reduce Transparency, Reduce Motion, VoiceOver focus, keyboard-only navigation, and large text. Deterministic XCUITest coverage now exercises the System/Light/Dark appearance launch seams and asserts the visible preview without changing persisted preferences; host/system and participant verification of appearance and the remaining accessibility settings remains open.
...
- [ ] Compare against Apple Foundation Models on the same evaluation corpus. The fail-closed `ModelComparator` now requires matching corpus revisions and finite safety/cost measurements, then ranks unsupported-claim rate, citation coverage, and cost; an eligible-device, consented run remains open.
...
- [x] Keep existing unit tests and add protocol-fake tests for every external/platform boundary. Stirling, translation, persistence, DOCX, PDF operations, screenshot command, AVSpeech synthesis, intelligence provider, and screen-capture permission/picker seams now have injectable fakes; real picker UI and system participant paths still require platform-specific coverage.
- [ ] Add XCUITest coverage for import, review queue, export confirmation, settings, dark/light appearance, and denied permission recovery. The deterministic `-ui-testing-fixture` flow now loads the bounded demo and covers Review Queue/Continue, the responsive Review issue/More/search controls, every export format, export option, summary/listening/citation/draft actions, and preview without invoking a file panel; `-ui-testing-export` now writes real Markdown/DOCX bytes to a temporary directory and asserts the export status; a `-ui-testing-settings` seam now covers privacy, searchable-copy, appearance, contrast, and retention controls; dedicated appearance, Stirling, import, and denied-import seams remain deterministic. Real picker/permission, native save-panel behavior, import/export execution in a live worker, and participant execution remain open.
...
- [ ] Run a final physical-Mac smoke test: PDF, image, clipboard, selected capture, window capture, translation, review queue, each export, optional Stirling compression/merge with cancellation and separate-output checks, deletion/retention controls, VoiceOver, light/dark appearance, and offline behaviour.
